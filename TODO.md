...
#### 2D.1 - prism-code-editor migration: editable source viewer, gutter markers, maximize, download

**STATUS: CodeMirror 6 attempted and abandoned (2026-08-02); switched to prism-code-editor.** A full CM6 implementation was built and got as far as a non-crashing, correctly-parsing, editable editor, but syntax highlighting never rendered despite many rounds of fixes (a jsDelivr URL-syntax bug, a cross-package `@codemirror/state` version-skew crash, a wrong tag mapping, then a still-unresolved highlighting-only failure isolated all the way down to "even `@codemirror/language`'s own fully self-contained built-in `defaultHighlightStyle` produces zero spans," in a from-scratch isolated test page with direct tokenizer/tree instrumentation). Root cause was never found. Given the time already spent, the decision was made to stop debugging CM6 and switch libraries rather than keep guessing; `index.html` was reverted to its pre-CM6 state rather than attempting a partial extraction (the toolbar/Maximize-dialog UI built alongside CM6 was written directly against CM6-specific concepts — `editorView.dom`, a `StateField`-backed marker system — closely enough that salvaging it would cost about as much as rewriting it against a different library's actual API).

Replaces the hand-rolled read-only viewer (`renderSourceViewer`, tokenizer `appendTokenizedLine`/`findCommentStart`, gutter markers `annotateSourceViewerOutputMarkers`/`placeMarker` — see `index.html` as of commit `794fa0f` for current line numbers, since they drift) with [prism-code-editor](https://github.com/jonpyt/prism-code-editor) (npm `prism-code-editor`, verified current version `5.3.0` against the npm registry and the package's own shipped `.d.ts`/`.js` files, not documentation summaries) mounted into `#source-code-lines`. Chosen specifically because it has **zero runtime dependencies** and every internal cross-file import is a relative path within the one published package — the entire class of bug that sank the CM6 attempt (two independently-CDN-versioned packages silently resolving a shared dependency to two different instances) is structurally unreachable here, since there is only ever one package, one version, one bundle.

1. Load lazily on first script-staged render via dynamic `import()` from a pinned CDN URL (`https://cdn.jsdelivr.net/npm/prism-code-editor@5.3.0`, plus its `/languages/r`, `/prism/languages/r`, and `/layout.css` subpaths) — same lazy pattern `getEngine()` already uses for webR. Use `createEditor()` directly, **not** the `basicEditor`/`minimalEditor` setup helpers — those render inside a shadow root and load theme CSS asynchronously, which would isolate the editor from this app's own stylesheet.
2. Line numbers are built in via a CSS `counter()` pseudo-element per line; there's no gutter-extension API to learn. `PrismEditor.lines` is a real `HTMLCollectionOf<HTMLDivElement>` indexed by line number — plot/file marker icons are added by directly appending a button (reusing the existing `.output-marker` class and `jumpToOutput()` calls) into `editor.lines[lineNumber]`. Carry over two lessons learned building this for CM6, since they apply regardless of library: cap markers per line with an overflow "+N" summary (a script whose `PLOT_CALL_RE`/`FILE_WRITE_CALL_RE` heuristic finds no candidates falls back to piling every output onto one line, which can bloat a DOM line badly), and when there are no candidates, spread outputs one-per-source-line rather than stacking them all on the chunk's first line so the cap is rarely hit at all.
3. Editable, and edits feed execution immediately: `editor.on("update", (value) => ...)` writes the current value back into the active script's staged bytes **in place** (never via `stagedStore.upsert`, which would re-render the file carousel on every keystroke) — this keeps `staged.bytes` always current, so Run and `writeStagedFilesToVfs` need no separate wiring to see live edits. Clear that script's gutter markers on the same edit, same reasoning as before.
4. Editor-header toolbar: a small flex row with (a) the clickable script-title button (wired in 2D.2), (b) a ghost "Maximize" icon button, (c) a ghost "Download" icon button, using the shared ghost-icon-button class — this UI shape doesn't depend on which editor library is inside it.
5. **Maximize**: reuse the app's existing `<dialog>` pattern (`load-script-modal`/`error-modal`) — native `showModal()` gives a real focus trap and Escape-to-close for free. Re-parent `editor.container` (a plain `<div>`, no virtualized-rendering concerns across the move) into the dialog's content on open and back on close.
6. **Download**: `editor.value` directly (no `state.doc.toString()`-equivalent needed) via the shared Blob+objectURL+`<a download>` pattern (`downloadBlob()`), named after the active script's own filename/extension. Factor into one `downloadActiveScript()` helper, reused verbatim by the fake "Report" file row's "Download Script" button in 2D.8.
7. `.Rmd` handling: same underlying problem as CM6 (markdown + YAML front matter + fenced R chunks is not R source) needs the same fix in spirit — don't run the whole buffer through the plain R grammar. Evaluate at implementation time between a small custom preprocessing step keyed off this app's own `RE_CHUNK_OPEN`/`RE_FENCE_CLOSE_ONLY` regexes (so highlighting and execution can never disagree about what counts as an R chunk) versus Prism's native embedded-language pattern (`insertBefore` with an `inside` grammar, the same mechanism Prism's bundled `markdown` language uses) if it can be adapted to `{r ...}`-style fence headers.

**Before writing any of this into `index.html`, build the same kind of isolated single-file test page used to diagnose the CM6 failure** (plain HTML, `createEditor()` + the R grammar imports + a sample of R code, self-reporting pass/fail in visible page text, served via `python -m http.server` since ZippyServe rejects new files) and confirm real syntax highlighting renders before integrating. This would have caught the CM6 problem in minutes instead of many rounds.

**Done when:** the editor is directly editable with working undo/redo (native textarea undo/redo, or `editHistory()` if that proves insufficient) and R syntax highlighting; 📊/📄 gutter icons jump to the right Plots/Files tab item; editing code and pressing Run executes the edited version; Maximize opens a real focus-trapped fullscreen dialog and Escape closes it cleanly; Download saves the current (possibly edited) script content under its original filename.

**STATUS: COMPLETE (2026-08-02).** Implemented against `prism-code-editor@5.3.0` per the plan above: lazy `import()` loaded via `getPrismCodeEditor()`, mounted into `#source-code-lines` with `createEditor()`; line numbers via the library's own CSS counters; 📊/📄 output markers appended directly into `editor.lines[n]`; edits write straight into the active staged entry's bytes on every `onUpdate` (no `stagedStore.upsert` re-render); the editor-header toolbar (Download/Maximize) uses the shared ghost-icon-button class; Maximize re-parents `editor.container` into the existing `<dialog>` pattern; `.Rmd` gets a small custom `rmd` grammar registered against this app's own chunk-fence regexes rather than fighting Prism's generic markdown embedding. Confirmed working by hand-refresh testing in the real app this session: mount + R syntax highlighting render correctly, gutter markers jump to the right tab item, editing and pressing Run executes the edited text, Maximize/Restore and Escape work, and Download saves the current in-editor content under the original filename. The script-title button (`#source-viewer-filename`) is deliberately left as plain non-interactive text — Stage 2D.2 turns it into the real popover trigger.
...
**STATUS: COMPLETE (2026-08-02).** Implemented as specified: `#source-viewer-filename` is now a real `popovertarget` button (chevron icon rotates open/closed, `aria-haspopup`/`aria-expanded`); the native `[popover]` element holds a JS-built nested `<ul>`/`<li>` tree grouped by `/`-splitting each staged script's path, folders as plain labels, files as buttons (`aria-current="true"` + `.selected` on the active one); selecting a file drives the exact same `App.explicitEntryPath`/`syncUrlBar`/`updateSelectedEntry()` transition the old `<select>` used; position is computed from the trigger's `getBoundingClientRect()` on `beforetoggle` (CSS anchor positioning isn't reliable enough across browsers yet to depend on); focus moves to the current entry on open and back to the trigger on close, except when a light-dismiss click already moved focus somewhere else, which is left alone rather than yanked back. One deliberate scope change from the spec text above: `#entry-candidates` (the "multiple scripts found, choose one" radio-button fieldset) was **also removed**, not left alone as originally planned — per direct follow-up instruction, `autoDetectEntry` now always resolves a single best-guess script (first match by the same root-first/`.R`-before-`.Rmd`/alphabetical sort that used to order the candidate list) and runs it immediately, since the researcher can now switch scripts via this popover instead of being blocked on an upfront choice. Whenever a repo is involved, every other script it contains is also staged (`stageAllScriptFiles`, reusing the same fetch-everything helper the old candidate UI relied on) — regardless of whether `entry=` was given explicitly or auto-detected — purely so the popover always has real switching options, not just whichever script(s) `source()`/`child=` dependency-walking from the one active entry happened to visit. Verification this session was code-level only (no browser automation available); a human still needs to confirm live in the app that: the popover opens/positions/focuses correctly, a repo with an ambiguous entry point auto-runs its best guess without ever showing a blocking picker, and every script in the repo (not just the ones related to the active entry) appears in the popover regardless of whether `entry=` was explicit.
...
**STATUS: COMPLETE (2026-08-03), pending human verification.** Implemented as specified, with library versions verified live against the npm registry (`data.jsdelivr.com`) and GitHub (license + last-push date via the GitHub API) rather than assumed: `docx@9.7.1`, `pptxgenjs@4.0.1`, `exceljs@4.4.0` (all MIT, all pushed to within the last year, none archived). All four load lazily via jsDelivr's `+esm` combinator, following the shared-infrastructure loader pattern (`getDocxLib`/`getPptxGenJsLib`, alongside `getEngine()`/`getPrismCodeEditor()`). A new `extractReportBlocks()`/`extractPlotImages()` pair walks the rendered Report/Plots DOM once into a normalized, library-agnostic block list, shared by all three document exporters (avoids three near-duplicate DOM walkers) — DOCX/ODT flatten it into one continuous flow, PPTX consumes it directly for its one-slide-per-top-level-block rule. The existing `renderReportAsSelfContainedHtml()` (previously only used by the ZIP export's `report.html`) gained a copyright footer and a `renderPlotsAsSelfContainedHtml()` sibling, both reused by the new Web-page button — a genuine improvement to the ZIP export, not scope creep. ODT is the hand-built ODF package as specified (`content.xml`/`styles.xml`/`META-INF/manifest.xml`/`Pictures/`), zipped with `fflate.zipSync`'s per-file `[data, {level}]` tuple form (confirmed present in the pinned 0.8.2 build's own TypeScript definitions) so `mimetype` can be the required first entry, stored uncompressed — a real ODF-spec constraint, not a style choice. Two real API surprises were caught by statically inspecting the actual downloaded `+esm` bundles (via `curl` + grep, since no headless browser is available in this environment) before writing any integration code against them, rather than trusting documentation: (1) ExcelJS's `+esm` bundle exposes only a `default` export (an object carrying `Workbook` etc.), not individual named exports; (2) jspdf-autotable's bundled auto-plugin side effect only patches a *global* `window.jsPDF`, which a dynamically-imported `jsPDF` class never becomes, so `doc.autoTable(...)` is never available -- the free `autoTable(doc, options)` function is used instead throughout (this also affects 2D.7 below). A new `scratch/doc-export-libs-test.html` (same isolated-test-page convention as 2D.1's CM6/prism-code-editor tests) builds one minimal real file per library against the exact pinned `+esm` URLs and offers each as a download, for a human to open in real Word/LibreOffice/PowerPoint/a PDF viewer. **Verification was code-level only** (static export-surface inspection of the downloaded bundles, a JS brace/paren/bracket balance check across the whole module script, and a check that every new identifier is declared exactly once) — no browser automation is available in this environment, so whether the four buttons actually produce correctly-formatted, opening-without-repair files still needs a human to confirm live in the app, per `AGENTS.md` section 11.
...
**STATUS: COMPLETE (2026-08-03), pending human verification.** Implemented as specified. `splitCsvLine`/`buildCsvPreviewTable` were generalized into `splitDelimitedLine`/`splitTabularRows`/`buildCsvPreviewTable(text, maxRows, delimiter)` so CSV and TSV share one parser across preview, XLSX, and PDF-table export rather than three. Preview generalizes to JSON (pretty-printed, falling back to raw text on a parse failure rather than throwing), JSONL (pretty-printed per line), and `.txt/.md/.log/.r/.rmd` (raw `textContent`, never `innerHTML`, matching this file's existing rule for R-derived text); images keep their existing always-on inline preview with no separate button, per spec. `jsPDF`'s image-PDF path reuses the row's own already-loaded inline `<img>` element directly (jsPDF's `addImage` accepts a loaded `HTMLImageElement`) rather than round-tripping through a data URL, and is scoped to `png/jpg/jpeg/webp` only — `svg/gif/bmp` get no PDF button (jsPDF has no reliable native path for them, "otherwise skip" applied per format). One inefficiency caught and fixed before finishing: the first draft created two separate `Blob`/`ObjectURL`s for an image file's inline preview and its Download link; `appendFileRowButtons` now creates one and reuses it for both. **Verification was code-level only** — no browser automation available; a human still needs to confirm live that each file type shows the right button subset and that XLSX/PDF outputs actually open correctly, per `AGENTS.md` section 11.
